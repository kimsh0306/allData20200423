			...
			*/
			...
			$.ajax({
				...
				,success:function(result){
					...
					imsi += "</table><br>";
					imsi += "<button id='btn_del_rList'>삭제</button><button id='btn_upd_rList'>수정</button>";
...
//					$("#d_resList").html(imsi);
		...
		function login(){
			...
			$("#f_login").attr("action","./login.mvc2?crud=login");
			//submit은 form 전송
			$("#f_login").submit();
				...
				<table width="100%" border="0" borderColor="blue">
					<tr>
						<td style="width:110px">
							<span id="logok"><%=s_name+"님 반갑습니다." %></span>
						</td>
						<td>
...
<!-- ------------------------------------------------------------------------------------------------------- -->	
	...
	<!--=================================================== 모달 창에 필요한 코드 끝  -->
	<script type="text/javascript">
	//http://localhost:5000/member/member.mvc2?crud=memberList
		...
		$('#dl_memList').dialog({
		    ...
		    height: 200,
		    closed: true,
		    cache: false,//캐시는 사용하지 않겠다. == 무조건 새로 요청하겠다.
