...
public class BaseBallGameView {
	...
	JMenuBar	jmb_game	= new JMenuBar();
	...
	//생성자의 파라미터로 배열의 주소번지(String result[])를 받게 되는데 이 값을 사용하는 곳이
	//생성자 안이 아니라 initDisplay메소드 안, setTitle메소드에서
	//사용해야 하기 때문에 파라미터로 넘어온 값을 반드시 전변과 초기화해야 합니다.
	...
	//생성자 안에서 메소드를 호출하면 인스턴스화 없이도 호출이 가능함.
		if(this.result[0]!=null) {	//null이 아닐 때 화면을 띄워라.
			initDisplay();
			bbLogic.ranCom();			
		}
	}
	...
	public void initDisplay() {
		...
		//메뉴를 메뉴바에 붙여요
		jmb_game.add(jm_game);
		jmb_game.add(jm_info);		
		setJMenuBar(jmb_game);
